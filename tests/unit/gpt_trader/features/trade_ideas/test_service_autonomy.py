...
def _corrupt_autonomy_log(tmp_path: Path) -> None:
    ...
    repository = StateRepository(tmp_path / "trade_ideas")
    repository.initialize()
    with sqlite3.connect(repository.path) as connection:
        connection.execute(
            "INSERT INTO events(stream,payload) VALUES('autonomy_state.jsonl','garbage')"
        )
...
def test_current_autonomy_adopts_concurrent_seed(service: TradeIdeaService) -> None:
    from concurrent.futures import ThreadPoolExecutor

    service._repository.initialize()
    other = TradeIdeaService(service.root)
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(pool.map(lambda client: client.current_autonomy(), [service, other]))
