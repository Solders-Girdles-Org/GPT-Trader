...
> Prefer **derived truth** (code, tests, GitHub issues) over **authored
> prose**. Author prose only for what cannot be derived:
...
| Architecture and standards | Authored docs for invariants (`docs/ARCHITECTURE.md`, `docs/DI_POLICY.md`, `docs/naming.md`, `docs/testing.md`) plus source code | Markdown prose for invariants; `rg` over the code for inventories | Docs scanners | A durable invariant or standard changes | Hand-maintained module inventories |
...
| Config profiles and templates | Tracked YAML/JSON config under `config/**`, `.env.template`, and typed config code | Docs pointers to the config code and template | Config tests | Profile, env var, or schema behavior changes | Untracked `.env`, docs tables as canonical config, runtime profile dumps, hand-maintained env-var inventories |
...
| What is built / current state | `docs/STATUS.md` (small, pointer-only) + the code/tests it points to | ARCHITECTURE prose, rubric stage tables, README |
...
| How the code is structured | `docs/ARCHITECTURE.md` + the code (`rg`) | Hand-maintained module inventories |
| API / CLI / env-var / metric inventories | The code itself (`rg -n` under `src/gpt_trader/`) | Hand-written or generated reference tables that must be kept in sync |
...
2. **Prefer derived over authored.** If a fact can be read from code, tests,
   or the issue tracker, do not also assert it in prose.
