...
|------|------------|----------------|
| `GuardManager` per-guard convenience methods (`log_guard_telemetry`, `guard_daily_loss`, `guard_liquidation_buffers`, `guard_mark_staleness`, `guard_risk_metrics`, `guard_volatility`, `guard_api_health`) | Unreleased | Test-only surface that addressed guards by list index. Call the guard class directly (`features/live_trade/execution/guards/`) or run the stack via `GuardManager.run_guards_for_state()`. |
...
| Generated agent context (`var/agents/**`, `scripts/agents/generate_*.py`, `regenerate_all.py`, `export_model_schemas.py`, `agent_artifacts.py`, `config/agents/flows/`) and the `agent-regenerate` / `agent-artifacts` / `agent-tests` CLIs | Unreleased | Retired (foundations Move 1, owner-approved 2026-09-16). Read the fact from its source: env vars via `rg -n "getenv\|environ" src/gpt_trader` and `config/environments/.env.template`; config/risk schemas in `src/gpt_trader/app/config/bot_config.py` and `features/live_trade/risk/config.py`; metrics in `monitoring/metrics_collector.py`; log events in `src/gpt_trader/logging/`; tests mirror `src/` paths. |
| `agent-check` (`quality_gate.py`), `agent-health` (`health_report.py`), `agent-impact` (`change_impact.py`), `agent-map` (`dependency_graph.py`) CLIs and the matching `agent-health-*` / `agent-impact*` Makefile targets | Unreleased | Retired with the agent-context layer. Use `uv run local-ci` (or the individual `ruff` / `black` / `mypy` / `pytest` commands) for quality checks and `rg` for import/impact questions. |
| `agent-dedupe` (`generate_dedupe_candidates.py`), `tests/_triage/dedupe_candidates.yaml`, `tests/_triage/dedupe_triage.yaml`, `scripts/ci/check_dedupe_manifest.py`, pre-commit `dedupe-manifest` hook | Unreleased | Retired with the agent-context layer. `tests/_triage/legacy_tests.yaml` and `check_legacy_test_triage.py` remain for legacy triage. |
| CI jobs `Agent Health` and `Agent Artifacts Freshness`; workflow `agent-artifacts-refresh.yml`; `local-ci --include-agent-health` and its advisory freshness step | Unreleased | Retired with the agent-context layer. The required CI set and `uv run local-ci` are unchanged. |
| `agent-pr-ready` artifact-freshness advisory and its skip flag | Unreleased | Removed with the generated context; `agent-pr-ready` still reports required checks, merge state, review threads, and branch-protection drift. |
...
| `daily_loss_limit` in profile schema | Unreleased | Removed with the profile appetite fields; the runtime breaker derives from the RiskBudget (#1120). |
