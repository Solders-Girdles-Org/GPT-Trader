...
## Finding the tests for a change

Unit tests mirror source paths (`src/gpt_trader/features/live_trade/bot.py` is
covered under `tests/unit/gpt_trader/features/live_trade/`). To find importers
of a module before a wider change:

```bash
rg -n "from gpt_trader.features.live_trade.bot import|gpt_trader.features.live_trade.bot" src tests
